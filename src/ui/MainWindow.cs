...
namespace anime_catalog_application
{
    public class MainWindow : Form
    {
        ...
        private bool connected_to_database(Database database)
        {
            if(!isConnected)
            {
                try
                {
                    isConnected = database._connect();
                    lbl_db_connection_status.Text = "Open";
                    lbl_db_connection_status.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(0)))), ((int)(((byte)(255)))), ((int)(((byte)(0)))));
                    return true;
                }
                catch(Exception ex)
                {
            ...
            {
                MessageBox.Show("Already connected to Database", "Error");
                return false;
            }   
            else
            {
        ...
        private bool close_connection_to_database(Database database)
        {
            if(isConnected)
            {
                database._close();
        ...
        private void btn_load_anime_series_Click(object sender, System.EventArgs e)
        {
            if(!isConnected)
            {
               MessageBox.Show("No Connection To Databse"); 
            }
            else if(isConnected)
            {
                lsb_anime_info.Items.Clear();
                 _database.load_database_into_listBox(lsb_anime_info);
            }
            else
            {
                MessageBox.Show("Anime Already Loaded into List Box");
            }
        ...
        {
            MessageBox.Show("Button to add Anime Series to the Database", "Button 2");
        }

        private void btn_show_unfinished_anime_series_Click(object sender, EventArgs e)
        {
            MessageBox.Show("This will only show anime that have not been finished or declared as being finished");
        }
        ...
        {
            if(connected_to_database(this._database))
            {
                MessageBox.Show("Connection Established to the Database");
            }
        }

        private void database_connectionCloseToolStripItem_Click(object sender, System.EventArgs e)
        {
            isConnected = close_connection_to_database(this._database);
